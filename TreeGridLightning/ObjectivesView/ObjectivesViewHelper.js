({
	getGrandParentObjectives : function(cmp) {
		var action = cmp.get('c.getGrandParentObjectives');
        action.setParams({searchText: cmp.get('v.searchText')});
        
        action.setCallback(this, $A.getCallback(function (response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                cmp.set('v.objectives', response.getReturnValue());
                var parentObjectiveIds = cmp.get('v.parentObjectiveIds');
                if(parentObjectiveIds && parentObjectiveIds.length>0){
                    this.getChildObjectives(cmp,parentObjectiveIds[0],true,1);
                }
            } else if (state === "ERROR") {
                var errors = response.getError();
                console.error(errors);
            }
            cmp.set("v.spinner","hide");
        }));
        
        $A.enqueueAction(action);
	},
    
    getChildObjectives : function(cmp, selectedObjectiveId, flag, expandLevel) {
        if(flag){
            ...
            action.setCallback(this, $A.getCallback(function (response) {
                ...
                if (state === "SUCCESS") {
                    ...
                    var parentObjectiveIds = cmp.get('v.parentObjectiveIds');
                    if(expandLevel>0 && parentObjectiveIds && parentObjectiveIds.length>expandLevel){
                        this.getChildObjectives(cmp,parentObjectiveIds[expandLevel].trim(),true,expandLevel+1);
                    }
                } else if (state === "ERROR") {
                    var errors = response.getError();
                    console.error(errors);
